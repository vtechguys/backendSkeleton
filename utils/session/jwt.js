...
function returnSessionDurationBasedOnRole(
  role,
  duration = 7,
  rememeberMe = false
) {
...
const jwtOperations = {
  generateJwt(id, role = "user", duration = 7) {
    ...
    let jwtDuration = returnSessionDurationBasedOnRole(
      role,
      duration,
      rememberMe
    );
  ...
  getSessionBySessionId(sessionId, callback) {
    ...
    SessionCRUD.getSessionBySessionId(
      sessionId,
      function utilsGetSessionBySessionIdCb(error, result) {
        if (error) {
          logger.error(error);
  ...
  fillJwtSession(userData, callback) {
    ...
    if (userData.userId) {
      ...
      SessionCRUD.removeAllSessionForThisUser(
        userData,
        function utilsFillJwtSessionCb1(error, result) {
          if (error) {
            logger.error(error);
            return callback(error, null);
          } else {
  ...
  storeSession(userData, callback) {
    ...
    SessionCRUD.createSession(userData, function utilsStoreSessionCb(
      error,
      result
    ) {
      if (error) {
