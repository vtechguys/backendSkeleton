...
const authenticate = {
  jwtSession(request, response, next) {
    ...
    const isRequestedUrlSimpleUrl =
      SIMPLE_URLS_ARRAY.indexOf(requestedUrl) > -1;
      ...
      if (tokenHeader) {
        ...
        if (isValidTokenFormat && token) {
          ...
          jwtOperations.getSessionBySessionId(token, (error, sessionResult) => {
            if (error) {
              ...
              if (sessionResult && sessionResult.sessionId) {
                ...
                jwt.verify(request.token, constants.JWT_KEY, function(
                  error1,
                  result1
                ) {
                  if (error1) {
          ...
          sendResponse.badRequest(
            response,
            "Invalid token format or missing token."
          );
        }
      } else {
